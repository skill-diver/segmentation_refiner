...
def check_single_loc_updated(prev_loc, loc, loc_threshold=1.5):
    import numpy as np
    
    # check 2 locations are NumPy
    prev_loc = np.array(prev_loc)
    loc = np.array(loc)
    # check 2 locations distances
    dist = np.linalg.norm(prev_loc - loc)
...
def masks_overlapping_pct(mask1, mask2):
    ...
    intersection = np.logical_and(mask1==1, mask2==1).astype(np.int32)
    union = np.logical_or(mask1==1, mask2==1).astype(np.int32)
...
def duplicated_locations_index(individual_mask_list): 
    ...
    for i in range(len(individual_mask_list)-1):
        ...
        overlapping = np.logical_and(current_mask==1, next_mask==1).astype(np.int32)
...
def locations_and_masks_without_duplication(loc_list, mask_list, loc_has_converged, loc_needs_refinement):
    ...
    locations, mask_list, loc_has_converged, loc_needs_refinement = remove_duplicated_locations_with_masks(loc_list, mask_list, 
                                                                                        ...
                                                                                        duplicated_locations_idx)
    locations = np.array(locations).astype(np.int32)
...
def per_location_refiner_iter(loc, pir_img, model_file_seg_idv, seg_idv_norm,
    ...
    while True:
        ...
        if binary_mask is not None:
            overlap_w_bi = np.logical_and(binary_mask, mask).astype(np.int32)
...
def update_locations_masks_labels(extra_masks, extra_locs, extra_refinement, extra_convergence,
    ...
    if len(locations) == 0:
        ...
        locations = np.concatenate((locations, np.array(extra_locs)), 0).astype(np.int32)
    ...
    binary_mask = np.logical_or(binary_mask, individual_binary_mask).astype(np.int32)
...
def _is_gap_fitting_regressor(gap_list, idx, group_label):
    ...
    gt_gap = gap_list[idx]
    print("in gap regression function")
    print(f"Checking gap at index {idx}: {gt_gap}")  # Debug statement
    print(f"Gap list length: {len(gap_list)}")  # Debug statement
...
def consistency_refinement_close_loop(locations, pir_img, binary_mask,
    ...
    locations = np.empty((0, 3)) if len(locations) == 0 else np.array(locations).reshape(-1, 3)
    vertebrae_size_dict = load_vertebrae_sizes_pkl()
    ...
    while True:
        ...
        if main_loop_counter == 1:
            ...
            for i, label in enumerate(existing_labels):
                loc = label['location']
                if i > 0:
                    prev_loc = existing_labels[i - 1]['location']
                    prev_gap = compute_dist(prev_loc, loc)
                    gap_list.append(prev_gap)
                    print(f"Gap between {i-1} and {i}: {prev_gap}")  # Debug statement
            print(f"Initial gap list: {gap_list}")  # Debug statement
            print(f"Initial gap list length: {len(gap_list)}")  # Debug statement
            # check the existing labels correctness
            for i, label in enumerate(existing_labels):
                mask, loc, index = label['mask'], label['location'], label['index']
                group_label = labels_to_group_labels(index)
                
                # Compute the gap as the distance to the previous and next vertebrae locations
                if i > 0:
                    prev_loc = existing_labels[i - 1]['location']
                    #prev_gap = gap_list[i - 1]
                    prev_gap = compute_dist(prev_loc, loc)
                    #gap_list.append(prev_gap)
                else:
                    prev_gap = None

                if i < len(existing_labels) - 1:
                    next_loc = existing_labels[i + 1]['location']
                    next_gap = compute_dist(loc, next_loc)
                else:
                    next_gap = None

                

                # Check if the gaps fit the Gaussian distribution
                if prev_gap is not None:
                    if_fit_prev_distr = _is_gap_fitting_gauss_distribution(prev_gap, group_label, cervical_coeff, thoracic_coeff, lumbar_coeff)
                else:
                    if_fit_prev_distr = [1, -1]  # If no previous gap, consider it as fitting

                if next_gap is not None:
                    if_fit_next_distr = _is_gap_fitting_gauss_distribution(next_gap, group_label, cervical_coeff, thoracic_coeff, lumbar_coeff)
                else:
                    if_fit_next_distr = [1, -1]  # If no next gap, consider it as fitting
                
                # Get size mean and std for the current label
                label_key = str(index)
                size_mean = vertebrae_size_dict[label_key]['mean']
                size_std = vertebrae_size_dict[label_key]['std']
                current_size = np.count_nonzero(mask == 1)
                
                fit_size = _in_frange(current_size, size_mean - 3 * size_std, size_mean + 3 * size_std)
                
                print("Current index:", i)  # Debug statement
                print("Current gap list length:", len(gap_list))  # Debug statement
                print("Gap list:", gap_list)  # Debug statement
                fit_gap_regressor = _is_gap_fitting_regressor(gap_list, i - 1, group_label) if i > 0 else [1, -1]
                
                
                if if_fit_prev_distr[0] == 1 and if_fit_next_distr[0] == 1 and fit_size and fit_gap_regressor[0] == 1:
                    print("passing index:", index)
                    print("passing i:", i)
                    
                    idv_mask_list.append(mask)
                    loc_array = np.array(loc).reshape(1, 3)
                    #print("Debug: loc =", loc, "Type:", type(loc))
                    locations = np.vstack([locations, loc_array]).astype(np.int32)
                    #print("Debug: locations =", locations, "Type:", type(locations))
                    labels = np.append(labels, index)
                    print(labels)
                    loc_has_converged.append(True)
                    loc_needs_refinement.append(False)
                    ...
                    labels = np.append(labels, index)
                    print(labels)
                    loc_has_converged.append(False)
                    loc_needs_refinement.append(True)
                    print("index not fit Gaussian distribution:", index)
        ...
        if len(loc_list) > 0:
            print("doing refining in epoch", main_loop_counter)
            ...
            binary_mask = np.logical_or(sliding_mask, individual_binary_mask).astype(np.int32)
            labels = labelling_2msk(sliding_mask, individual_binary_mask, locations, loc_has_converged, 
                                    model_file_id_group, model_file_id_cer, model_file_id_thor, model_file_id_lum)
    ...
    print("Final locations before assertion:")
    print(locations)  
    print("Locations shape:", locations.shape)
    print("Locations data type:", locations.dtype)
    zero_value_indices = np.argwhere(locations == 0)
    if zero_value_indices.size > 0:
        print("Zero values found in locations at indices:")
